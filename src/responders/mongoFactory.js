...
const setPaths = () =>{
  ...
  tplDir = path.join(path.dirname(currentPath), 'src', 'tpl');  
...
mongoFactory.setConfig = async (configOptions) => {  
  ...
  if(!helpers.readDir(configOptions.dest)){
    ...
    console.log(helpers.red, `Path ${configOptions.dest} not found.`);
...
mongoFactory.create = async (dest, fileName) => {
  ...
  if(!helpers.readDir(dest)){
    ...
    console.log(helpers.red, `Path ${dest} not found.`);
